...
checkpoint_callback = CheckpointCallback(
        ...
        save_path = f'../models/{map_name}/PPO/checkpoints/PPO_{steps}steps_lr{LR}_{color}_FS{FS}_{action_wrapper}',
        name_prefix = 'step_')
...
model.learn(
        ...
        log_interval = 500,
        tb_log_name = f"PPO_{steps}steps_lr{LR}_{color}_FS{FS}_{action_wrapper}"
        )

# Save trained model
model.save(f"../models/{map_name}/PPO/PPO_{steps}steps_lr{LR}_{color}_FS{FS}_{action_wrapper}")
env.close()
