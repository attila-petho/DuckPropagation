...
checkpoint_callback = CheckpointCallback(
        save_freq=100000,
        save_path=f'../models/{map_name}/A2C/checkpoints/A2C_{steps}steps_lr{LR}_{color}_FS{FS}/',
        name_prefix='step_')
...
model.learn(
        ...
        log_interval=500,
        tb_log_name=f"A2C_{steps}steps_lr{LR}_{color}_FS{FS}"
        )

# Save trained model
model.save(f"../models/{map_name}/A2C/A2C_{steps}steps_lr{LR}_{color}_FS{FS}")
env.close()
